...
// RunDiagnostic runs the P2P diagnostic
...
func RunDiagnostic(
	...
	logger.Warn().Msg("P2P Diagnostic mode enabled")
	logger.Warn().Msgf("seed peer: %s", peers)
	...
	if err != nil {
		return errors.Wrap(err, "unable to get pub key from hotkey")
	}

	logger.Warn().Msgf("my pubkey %s", pubKeyBech32)

	var s *metrics.TelemetryServer
	if len(peers) == 0 {
		logger.Warn().Msg("No seed peer specified; assuming I'm the host")
	}

	p2pPriKey, err := crypto.UnmarshalSecp256k1PrivateKey(priKey[:])
	if err != nil {
		logger.Error().Err(err).Msg("UnmarshalSecp256k1PrivateKey error")
		return err
	}
	listenAddress, err := maddr.NewMultiaddr(fmt.Sprintf("/ip4/0.0.0.0/tcp/%d", 6668))
	if err != nil {
		logger.Error().Err(err).Msg("NewMultiaddr error")
		return err
	}
	IP := os.Getenv("MYIP")
	if len(IP) == 0 {
		logger.Warn().Msg("empty env MYIP")
	}
	...
	if err != nil {
		logger.Error().Err(err).Msg("fail to create host")
		return err
	}
	logger.Info().Msgf("host created: ID %s", host.ID().String())
	if len(peers) == 0 {
		s = metrics.NewTelemetryServer()
		s.SetP2PID(host.ID().String())
		go func() {
			logger.Info().Msg("Starting TSS HTTP Server...")
	...
	handleStream := func(s network.Stream) {
		...
		n, err := s.Read(buf)
		if err != nil {
			logger.Error().Err(err).Msg("read stream error")
			return
		}
		// send the message back
		if _, err := s.Write(buf[:n]); err != nil {
			logger.Error().Err(err).Msg("write stream error")
			return
		}
	}
	...
	logger.Info().Msg("Bootstrapping the DHT")
	...
	for _, peerAddr := range peers {
		...
		if err != nil {
			logger.Error().Err(err).Msgf("fail to parse peer address %s", peerAddr)
			continue
		}
		wg.Add(1)
		go func() {
			...
			if err := host.Connect(context.Background(), *peerinfo); err != nil {
				logger.Warn().Msgf("Connection failed with bootstrap node: %s", *peerinfo)
			} else {
				logger.Info().Msgf("Connection established with bootstrap node: %s", *peerinfo)
			}
		}()
	}
	wg.Wait()
	...
	for range ticker.C {
		...
		logger.Info().Msgf("Searching for other peers...")
		...
		for peer := range peerChan {
			peerCount++
			if peer.ID == host.ID() {
				logger.Info().Msgf("Found myself #(%d): %s", peerCount, peer)
				continue
			}
			logger.Info().Msgf("Found peer #(%d): %s; pinging the peer...", peerCount, peer)
			stream, err := host.NewStream(context.Background(), peer.ID, protocol.ID(ProtocolID))
			if err != nil {
				logger.Error().Err(err).Msgf("fail to create stream to peer %s", peer)
				continue
			}
			...
			if err != nil {
				logger.Error().Err(err).Msgf("fail to write to stream to peer %s", peer)
				err = stream.Close()
				if err != nil {
					logger.Warn().Err(err).Msgf("fail to close stream to peer %s", peer)
				}
				continue
			}
			...
			if err != nil {
				logger.Error().Err(err).Msgf("fail to read from stream to peer %s", peer)
				err = stream.Close()
				if err != nil {
					logger.Warn().Err(err).Msgf("fail to close stream to peer %s", peer)
				}
			...
			logger.Debug().Msgf("echoed message: %s", string(buf[:nr]))
			err = stream.Close()
			if err != nil {
				logger.Warn().Err(err).Msgf("fail to close stream to peer %s", peer)
			}
			...
			logger.Info().Msgf("ping-pong success with peer #(%d): %s;", peerCount, peer)
			okPingPongCount++
		}
		logger.Info().
