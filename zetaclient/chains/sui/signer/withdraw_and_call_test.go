...
func Test_getWithdrawAndCallObjectRefs(t *testing.T) {
	...
	// re-enable this unit test after re-enabling authenticated call
	t.Skip("skipping authenticated call tests for now")
...
// TODO: https://github.com/zeta-chain/node/issues/4066
// remove this test after re-enabling authenticated call
func Test_getWithdrawAndCallObjectRefsLegacy(t *testing.T) {
	// create test objects references
	gatewayID, err := sui.ObjectIdFromHex(sample.SuiAddress(t))
	require.NoError(t, err)
	withdrawCapID, err := sui.ObjectIdFromHex(sample.SuiAddress(t))
