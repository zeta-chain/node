...
func gasFromCCTX(cctx *types.CrossChainTx, logger zerolog.Logger) (Gas, error) {
	...
	} else if limit < contractCallMinGasLimit && (params.CoinType != coin.CoinType_Gas ||
		(!cctx.IsCurrentOutboundRevert() && cctx.InboundParams.IsCrossChainCall) ||
		(cctx.IsCurrentOutboundRevert() && cctx.RevertOptions.CallOnRevert)) {
