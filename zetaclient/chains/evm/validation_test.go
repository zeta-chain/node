...
func TestCheckEvmTransactionTable(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			tx:   testutils.LoadEVMInbound(t, TestDataDir, chainID, intxHash, coin.CoinType_Gas),
			fail: false,
		},
		{
...
func TestCheckEvmTransaction(t *testing.T) {
	...
	t.Run("should pass for valid transaction", func(t *testing.T) {
		tx := testutils.LoadEVMInbound(t, TestDataDir, 1, intxHash, coin.CoinType_Gas)
		err := evm.ValidateEvmTransaction(tx)
		require.NoError(t, err)
	})
	t.Run("should fail for nil transaction", func(t *testing.T) {
		err := evm.ValidateEvmTransaction(nil)
