...
func (signer *Signer) SignWithdrawTx(
	...
	inAmounts, err := signer.AddTxInputs(tx, prevOuts)
	if err != nil {
		return nil, err
	}
	...
	txSize, err := bitcoin.EstimateOutboundSize(int64(len(prevOuts)), []btcutil.Address{txData.to})
	if err != nil {
		return nil, err
	}
	if txData.txSize < bitcoin.BtcOutboundBytesWithdrawer { // ZRC20 'withdraw' charged less fee from end user
...
func (signer *Signer) AddTxInputs(tx *wire.MsgTx, utxos []btcjson.ListUnspentResult) ([]int64, error) {
	...
	for i, utxo := range utxos {
		...
		amount, err := bitcoin.GetSatoshis(utxos[i].Amount)
		if err != nil {
			return nil, err
		}
		amounts[i] = amount
...
func (signer *Signer) AddWithdrawTxOutputs(
	...
	payToSelfScript, err := signer.PkScriptTSS()
	if err != nil {
		return err
	}
...
// SignTx signs the tx with the given witnessHashes
func (signer *Signer) SignTx(
	...
	// get the TSS pkScript
	pkScript, err := signer.PkScriptTSS()
	if err != nil {
		return err
	}
