...
func TestInbound(t *testing.T) {
	t.Run("No gateway provided", func(t *testing.T) {
		...
		tonRepo := repo.NewTONRepo(ts.rpc, nil, ts.baseObserver.Chain())
		_, err := New(ts.baseObserver, tonRepo, nil)
		require.Error(t, err)
	})
	...
	t.Run("Donation", func(t *testing.T) {
		...
		// Given observer
		tonRepo := repo.NewTONRepo(ts.rpc, ts.gateway, ts.baseObserver.Chain())
		ob, err := New(ts.baseObserver, tonRepo, ts.gateway)
