...
func (ob *BTCChainClient) WithChain(chain pkg.Chain) {
	ob.Mu.Lock()
	defer ob.Mu.Unlock()
	ob.chain = chain
}
...
func NewBitcoinClient(
	...
	ob.chain = chain
	netParams, err := pkg.BitcoinNetParamsFromChainID(ob.chain.ChainId)
	if err != nil {
...
func (ob *BTCChainClient) postBlockHeader(tip int64) error {
	...
	_, err = ob.zetaClient.PostAddBlockHeader(
		...
		blockHash[:],
		res2.Block.Height,
		pkg.NewBitcoinHeader(headerBuf.Bytes()),
	)
...
func (ob *BTCChainClient) IsInTxRestricted(inTx *BTCInTxEvnet) bool {
	receiver := ""
	parsedAddress, _, err := pkg.ParseAddressAndData(hex.EncodeToString(inTx.MemoBytes))
...
func (ob *BTCChainClient) FetchUTXOS() error {
	...
	address, err := pkg.DecodeBtcAddress(tssAddr, ob.chain.ChainId)
	if err != nil {
...
func (ob *BTCChainClient) findNonceMarkUTXO(nonce uint64, txid string) (int, error) {
	...
	amount := pkg.NonceMarkAmount(nonce)
...
func (ob *BTCChainClient) checkTSSVout(params *types.OutboundTxParams, vouts []btcjson.Vout) error {
	...
	for _, vout := range vouts {
		...
		if vout.N == 0 {
			...
			if amount != pkg.NonceMarkAmount(nonce) {
				return fmt.Errorf("checkTSSVout: nonce-mark amount %d not match nonce-mark amount %d", amount, pkg.NonceMarkAmount(nonce))
			}
		}
...
func (ob *BTCChainClient) checkTSSVoutCancelled(params *types.OutboundTxParams, vouts []btcjson.Vout) error {
	...
	for _, vout := range vouts {
		...
		if vout.N == 0 {
			...
			if amount != pkg.NonceMarkAmount(nonce) {
				return fmt.Errorf("checkTSSVoutCancelled: nonce-mark amount %d not match nonce-mark amount %d", amount, pkg.NonceMarkAmount(nonce))
			}
		}
