		...
		if bn > lastBlockNum { // we have a new block
			...
			for _, chain := range chains {
				...
				if len(outSendList) > 0 {
					...
					sendListGroups := make([][]*types.CrossChainTx, numGroups)
...
func (co *CoreObserver) TryProcessOutTxBatch(sendBatch []*types.CrossChainTx, outTxMan *OutTxProcessorManager, targetChain string) {
	nonces := make([]uint64, len(sendBatch))
	for i, send := range sendBatch {
		nonces[i] = send.OutBoundTxParams.OutBoundTxTSSNonce
	}
	myid := co.bridge.keys.GetSignerInfo().GetAddress().String()
	logger := co.logger.With().
	...
	signers, err := co.bridge.GetObserverList(toChain, zetaObserverModuleTypes.ObservationType_OutBoundTx.String())
	if err != nil {
		logger.Warn().Err(err).Msgf("unable to get observer list: chain %d observation %s", toChain, zetaObserverModuleTypes.ObservationType_OutBoundTx.String())
...
func trimSends(sends []*types.CrossChainTx) int {
	...
	for i := len(sends) - 1; i >= 1; i-- {
		...
		if sends[i].OutBoundTxParams.OutBoundTxTSSNonce > sends[i-1].OutBoundTxParams.OutBoundTxTSSNonce+50 {
			start = i
			break
		}
	}
