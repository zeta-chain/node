		...
		if bn > lastBlockNum { // we have a new block
			...
			for chain, sendList := range sendMap {
				...
				for idx, send := range sendList {
					...
					included, confirmed, err := ob.IsSendOutTxProcessed(send.Index, int(send.Nonce), send.SenderChain == "ZETA" || send.ReceiverChain == "ZETA")
					if err != nil {
...
func (co *CoreObserver) TryProcessOutTx(send *types.Send, sinceBlock int64, outTxMan *OutTxProcessorManager) {
	...
	included, confirmed, _ := co.clientMap[toChain].IsSendOutTxProcessed(send.Index, int(send.Nonce), send.SenderChain == "ZETA" || send.ReceiverChain == "ZETA")
	if included || confirmed {
