...
func (k Keeper) GetTssAddress(goCtx context.Context, req *types.QueryGetTssAddressRequest) (*types.QueryGetTssAddressResponse, error) {
	...
	ethAddress, err := pkg.GetTssAddrEVM(tss.TssPubkey)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	bitcoinParams := pkg.BitcoinRegnetParams
	if req.BitcoinChainId != 0 {
		bitcoinParams, err = pkg.BitcoinNetParamsFromChainID(req.BitcoinChainId)
		if err != nil {
			return nil, status.Error(codes.Internal, err.Error())
		}
	}
	btcAddress, err := pkg.GetTssAddrBTC(tss.TssPubkey, bitcoinParams)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
