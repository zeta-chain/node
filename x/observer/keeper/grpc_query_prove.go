...
func (k Keeper) Prove(c context.Context, req *types.QueryProveRequest) (*types.QueryProveResponse, error) {
	...
	blockHash, err := pkg.StringToHash(req.ChainId, req.BlockHash)
	if err != nil {
	...
	if err != nil && !pkg.IsErrorInvalidProof(err) {
		return nil, status.Error(codes.Internal, err.Error())
	}
	if err == nil {
		if pkg.IsEVMChain(req.ChainId) {
			...
			proven = true
		} else if pkg.IsBitcoinChain(req.ChainId) {
