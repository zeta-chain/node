...
func TestChainParamsList_Validate(t *testing.T) {
	...
	t.Run("should return error for duplicated chain ID", func(t *testing.T) {
		...
		err := list.Validate()
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "duplicated chain id")
	})
}
...
func TestChainParamsEqual(t *testing.T) {
	...
	assert.True(t, types.ChainParamsEqual(*params.ChainParams[0], *params.ChainParams[0]))
	assert.False(t, types.ChainParamsEqual(*params.ChainParams[0], *params.ChainParams[1]))
}
