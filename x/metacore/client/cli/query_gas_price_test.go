...
func networkWithGasPriceObjects(t *testing.T, n int) (*network.Network, []*types.GasPrice) {
	...
	for i := 0; i < n; i++ {
		state.GasPriceList = append(state.GasPriceList, &types.GasPrice{Creator: "ANY", Index: strconv.Itoa(i), Prices: map[string]uint64{}, BlockNum: map[string]uint64{}, Supply: map[string]*types.GasPrice_ValueBlockPair{}, MedianSupply: &types.GasPrice_ValueBlockPair{}})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.GasPrice)
			}
		})
	}
}
...
func TestListGasPrice(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.GasPrice[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.GasPrice)
	})
}
