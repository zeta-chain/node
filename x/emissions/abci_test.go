...
func TestEndBlocker(t *testing.T) {
	t.Run("successfully process all emissons created in a block", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			totalAmount = totalAmount.Add(emission.Amount)
		}

		err := sk.BankKeeper.MintCoins(ctx, emissionstypes.UndistributedObserverRewardsPool, sdk.NewCoins(sdk.NewCoin(config.BaseDenom, totalAmount)))
		require.NoError(t, err)

		require.ElementsMatch(t, emissions, k.GetAllWithdrawEmissions(ctx))
		emissionsModule.EndBlocker(ctx, *k)
		require.Empty(t, k.GetAllWithdrawEmissions(ctx))
		for _, emission := range emissions {
			address, err := sdk.AccAddressFromBech32(emission.Address)
			require.NoError(t, err)
			balance := k.GetBankKeeper().GetBalance(ctx, address, config.BaseDenom)
			require.Equal(t, emission.Amount.String(), balance.Amount.String())
		}
	})

	t.Run("do not commit unsuccessful withdraws to state", func(t *testing.T) {
		k, ctx, sk, _ := keepertest.EmissionsKeeper(t)
		...
		for i, emission := range emissions {
			...
			balance := k.GetBankKeeper().GetBalance(ctx, address, config.BaseDenom)
			if i == unsuccessfulIndex {
				require.Equal(t, sdkmath.ZeroInt().String(), balance.Amount.String())
				continue
			}
			require.Equal(t, emission.Amount.String(), balance.Amount.String())
		}
	...
	t.Run("unable to process withdraw if address is invalid", func(t *testing.T) {
		...
		emission := sample.WithdrawEmission(t)
		emission.Address = "invalid_address"
		k.SetWithdrawEmissions(ctx, emission)
		k.SetWithdrawableEmission(ctx, emissionstypes.WithdrawableEmissions{
			Address: emission.Address,
			Amount:  emission.Amount,
