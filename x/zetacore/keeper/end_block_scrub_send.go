...
func (k Keeper) ScrubGasPriceOfStuckOutTx(goCtx context.Context) {
	...
	if height%100 == 0 { // every 100 blocks, roughly 10min
		scrubingStatus := []types.SendStatus{types.SendStatus_PendingOutbound, types.SendStatus_PendingRevert}
...
func (k Keeper) ScrubUtility(ctx sdk.Context, store sdk.KVStore, p []byte) {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if send.Status == types.SendStatus_PendingOutbound || send.Status == types.SendStatus_PendingRevert {
			if ctx.BlockHeight()-int64(send.FinalizedMetaHeight) > 100 { // stuck send
				var chain string
