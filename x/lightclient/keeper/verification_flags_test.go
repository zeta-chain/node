...
func TestKeeper_GetAllVerificationFlags(t *testing.T) {
	t.Run("can get all verification flags", func(t *testing.T) {
		...
		allVf := k.GetAllVerificationFlags(ctx)
		require.Len(t, allVf, 2)
		require.Equal(t, vf, allVf)
	})
