...
func EmitEventInboundFinalized(ctx sdk.Context, cctx *types.CrossChainTx) {
	...
	err := ctx.EventManager().EmitTypedEvents(&types.EventInboundFinalized{
		...
		SenderChain:    pkg.GetChainFromChainID(cctx.InboundTxParams.SenderChainId).ChainName.String(),
		...
		ReceiverChain:  pkg.GetChainFromChainID(currentOutParam.ReceiverChainId).ChainName.String(),
