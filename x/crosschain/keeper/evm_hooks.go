...
func (k Keeper) ProcessWithdrawalEvent(ctx sdk.Context, logs []*ethtypes.Log, contract ethcommon.Address, creator string) error {
	...
	if found {
		...
		msg := zetacoretypes.NewMsgSendVoter(creator, contract.Hex(), common.ZETAChain.String(), creator, toAddr, receiverChain, event.Value.String(), "", "", event.Raw.TxHash.String(), event.Raw.BlockNumber, 90000, coinType)
		sendHash := msg.Digest()
