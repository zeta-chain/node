...
func (k Keeper) ProcessZRC20WithdrawalEvent(ctx sdk.Context, event *zrc20.ZRC20Withdrawal, emittingContract ethcommon.Address, txOrigin string, tss observertypes.TSS) error {
	...
	senderChain, err := pkg.ZetaChainFromChainID(ctx.ChainID())
	if err != nil {
...
func (k Keeper) ProcessCCTX(ctx sdk.Context, cctx types.CrossChainTx, receiverChain *pkg.Chain) error {
...
func ValidateZrc20WithdrawEvent(event *zrc20.ZRC20Withdrawal, chainID int64) error {
	...
	if pkg.IsBitcoinChain(chainID) {
		...
		addr, err := pkg.DecodeBtcAddress(string(event.To), chainID)
		if err != nil {
