...
func TestKeeper_IterateAndUpdateCctxGasPrice(t *testing.T) {
	...
	supportedChains := []*pkg.Chain{
		{ChainId: pkg.EthChain().ChainId},
		{ChainId: pkg.BtcMainnetChain().ChainId},
		{ChainId: pkg.BscMainnetChain().ChainId},
		{ChainId: pkg.ZetaChainMainnet().ChainId},
	}

	// set pending cctx
	tss := sample.Tss()
	zk.ObserverKeeper.SetTSS(ctx, tss)
	createCctxWithNonceRange(t, ctx, *k, 10, 15, pkg.EthChain().ChainId, tss, zk)
	createCctxWithNonceRange(t, ctx, *k, 20, 25, pkg.BtcMainnetChain().ChainId, tss, zk)
	createCctxWithNonceRange(t, ctx, *k, 30, 35, pkg.BscMainnetChain().ChainId, tss, zk)
	createCctxWithNonceRange(t, ctx, *k, 40, 45, pkg.ZetaChainMainnet().ChainId, tss, zk)
