...
func (k Keeper) VerifyOutTxBody(ctx sdk.Context, msg *types.MsgAddToOutTxTracker, txBytes []byte) error {
	...
	if pkg.IsBitcoinChain(msg.ChainId) {
	...
	} else if pkg.IsBitcoinChain(msg.ChainId) {
...
func VerifyBTCOutTxBody(msg *types.MsgAddToOutTxTracker, txBytes []byte, tssBtc string) error {
	...
	for _, vin := range tx.MsgTx().TxIn {
		...
		bitcoinNetParams, err := pkg.BitcoinNetParamsFromChainID(msg.ChainId)
		if err != nil {
	...
	if tx.MsgTx().TxOut[0].Value != pkg.NonceMarkAmount(msg.Nonce) {
		return fmt.Errorf("want nonce mark %d, got %d", tx.MsgTx().TxOut[0].Value, pkg.NonceMarkAmount(msg.Nonce))
	}
