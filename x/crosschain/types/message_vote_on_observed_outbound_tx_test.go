...
func TestMsgVoteOnObservedOutboundTx_Digest(t *testing.T) {
	...
	hash2 = msg2.Digest()
	assert.Equal(t, hash, hash2, "status should not change hash")

	// cctx hash used
	msg2 = msg
	msg2.CctxHash = sample.StringRandom(r, 32)
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "cctx hash should change hash")

	// observed outbound tx hash used
	msg2 = msg
	msg2.ObservedOutTxHash = sample.StringRandom(r, 32)
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "observed outbound tx hash should change hash")

	// observed outbound tx block height used
	msg2 = msg
	msg2.ObservedOutTxBlockHeight = 43
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "observed outbound tx block height should change hash")

	// observed outbound tx gas used used
	msg2 = msg
	msg2.ObservedOutTxGasUsed = 43
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "observed outbound tx gas used should change hash")

	// observed outbound tx effective gas price used
	msg2 = msg
	msg2.ObservedOutTxEffectiveGasPrice = math.NewInt(43)
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "observed outbound tx effective gas price should change hash")

	// observed outbound tx effective gas limit used
	msg2 = msg
	msg2.ObservedOutTxEffectiveGasLimit = 43
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "observed outbound tx effective gas limit should change hash")

	// zeta minted used
	msg2 = msg
	msg2.ValueReceived = math.NewUint(43)
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "zeta minted should change hash")

	// out tx chain used
	msg2 = msg
	msg2.OutTxChain = 43
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "out tx chain should change hash")

	// out tx tss nonce used
	msg2 = msg
	msg2.OutTxTssNonce = 43
	hash2 = msg2.Digest()
	assert.NotEqual(t, hash, hash2, "out tx tss nonce should change hash")
