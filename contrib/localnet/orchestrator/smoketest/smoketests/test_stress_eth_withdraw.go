...
func TestStressEtherWithdraw(sm *runner.SmokeTestRunner) {
	...
	//// set the starting nonce since we're sending txs before getting receipts
	//nonce, err := sm.ZevmClient.PendingNonceAt(sm.Ctx, sm.DeployerAddress)
	//if err != nil {
	//	panic(err)
	//}
	...
	for i := 0; i < numWithdraws; i++ {
		i := i
		//sm.ZevmAuth.Nonce = new(big.Int).SetUint64(nonce)
...
func MonitorEtherWithdraw(sm *runner.SmokeTestRunner, tx *ethtypes.Transaction, index int, startTime time.Time) error {
	...
	if cctx.CctxStatus.Status != crosschaintypes.CctxStatus_OutboundMined {
		//sm.Logger.Info(
		//	"index %d: withdraw cctx failed with status %s, message %s",
		//	index,
