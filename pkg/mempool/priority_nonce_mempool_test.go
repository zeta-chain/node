...
func (s *MempoolTestSuite) TestRandomGeneratedTxs() {
	s.iterations = 0
	s.mempool = zetamempool.NewPriorityMempool(zetamempool.PriorityNonceWithOnRead(func(tx sdk.Tx) {
		s.iterations++
	}))
	t := s.T()
	ctx := sdk.NewContext(nil, cmtproto.Header{}, false, log.NewNopLogger())
...
func TestNextSenderTx_TxLimit(t *testing.T) {
	...
	// unlimited
	mp := zetamempool.NewPriorityMempool(zetamempool.PriorityNonceWithMaxTx(0))
	for i, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		require.NoError(t, mp.Insert(c, tx))
		require.Equal(t, i+1, mp.CountTx())
	...
	// limit: 3
	mp = zetamempool.NewPriorityMempool(zetamempool.PriorityNonceWithMaxTx(3))
	for i, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		err := mp.Insert(c, tx)
		if i < 3 {
	...
	// disabled
	mp = zetamempool.NewPriorityMempool(zetamempool.PriorityNonceWithMaxTx(-1))
	for _, tx := range txs {
		c := ctx.WithPriority(tx.priority)
		err := mp.Insert(c, tx)
		require.NoError(t, err)
