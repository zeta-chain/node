...
export class EVMChain extends Blockchain {
  ...
  async loadMPIContractABI() {
    ...
    if (this.name === "ethLocalNet") {
      contractName = "evm/ZetaConnector.eth.sol/ZetaConnectorEth.json";
    } else {
      contractName = "evm/ZetaConnector.non-eth.sol/ZetaConnectorNonEth.json";
    }
    return require(`../../../../zeta-contracts/packages/protocol-contracts/artifacts/contracts/${contractName}`).abi;
  }
