...
// const zeta = new ZetaChain("zeta-testnet", "http://44.199.226.6:1317", 1317);

let approvalTest;
let transferTest;
let messageSendTest;
let txMiningTest;
let checkForMPIMessages;

// Start Mocha Tests Here Calling Test Functions in Parallel
describe("Testing MockMPI Server", () => {
  it("Check EVM RPC Endpoints are responding", async () => {
    for (const network of [eth, bsc, polygon]) {
      try {
        ...
        console.debug(`Checking ${network.rpcEndpoint}`);
        if (response.status !== 200) {
          console.error(`${network.name} RPC Endpoint Not Responding`);
        }
  ...
  it("MPI contract can send() messages", async () => {
    ...
    messageSendTest = await Promise.all([
      testSendMessage(eth, bsc),
      testSendMessage(eth, polygon, true),
      testSendMessage(bsc, eth),
      testSendMessage(bsc, polygon, true),
      testSendMessage(polygon, eth),
      testSendMessage(polygon, bsc, true),
    ]);
    await messageSendTest;
  });

  it("MPI message events are received on destination Chain", async () => {
    await messageSendTest;
    console.info("Waiting 2.5 minutes for TXs to be processed");
    await wait(150000);

    checkForMPIMessages = await Promise.all([
      checkForMpiTransactions(
        eth,
        tssKey,
    ...
    for (const result of await checkForMPIMessages) {
      assert.equal(
        result,
        true,
        "No Successful Messages From TSS Address Detected To The MPI Contract Have Been Detected"
      );
    }
  });
});